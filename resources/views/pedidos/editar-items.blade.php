...
@section('contenido')
    <a href="{{ $volverUrl }}" class="text-sm text-slate-500 hover:underline">&larr; Volver</a>
    ...
    <form method="POST" action="{{ $actionUrl }}" id="form_editar_items" class="mt-4">
        @csrf
        ...
        <div class="flex gap-2">
            ...
            <a href="{{ $volverUrl }}" class="rounded px-4 py-2 text-sm font-medium text-slate-600 hover:underline">
                Cancelar
            </a>
        </div>
    </form>
    ...
    <script>
        (function () {
            ...
            const form = document.getElementById('form_editar_items');
                ...
                document.querySelectorAll('.btn-quitar-fila').forEach(function (boton) {
                    boton.onclick = function () {
                        if (cuerpo.querySelectorAll('tr').length <= 1) {
                            alert('Un pedido necesita al menos 1 producto.');
                            return;
                        }
            ...
            input.addEventListener('keydown', function (evento) {
                if (evento.key === 'Enter') {
                    ...
                    const escaneado = productos.find(function (producto) {

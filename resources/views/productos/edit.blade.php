        ...
        <form method="POST" action="{{ route('productos.update', $producto) }}" class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input id="codigo" name="codigo" type="text" value="{{ old('codigo', $producto->codigo) }}"
                        class="w-full rounded border border-slate-300 focus:border-slate-500 focus:ring-slate-500">
                </div>
                <div>
                    <label for="categoria" class="block text-sm font-medium mb-1">Categoría</label>
                    <input id="categoria" name="categoria" type="text" value="{{ old('categoria', $producto->categoria) }}"
                        class="w-full rounded border border-slate-300 focus:border-slate-500 focus:ring-slate-500">
                </div>
            </div>
            ...
            <div>
                ...
                <input id="marca" name="marca" type="text" value="{{ old('marca', $producto->marca) }}"
                    class="w-full rounded border border-slate-300 focus:border-slate-500 focus:ring-slate-500">
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input id="costo" name="costo" type="number" step="0.01" min="0" value="{{ old('costo', $producto->costo) }}"
                        class="w-full rounded border border-slate-300 focus:border-slate-500 focus:ring-slate-500">
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                @if (auth()->user()->empresa->controla_stock)
                    <div>
                        ...
                        <input id="stock" name="stock" type="number" step="1" min="0" value="{{ old('stock', $producto->stock) }}"
                            class="w-full rounded border border-slate-300 focus:border-slate-500 focus:ring-slate-500">
                    </div>
                @else
                ...
                <div class="@if (! auth()->user()->empresa->controla_stock) col-span-2 @endif">
                    ...
                    <input id="unidad" name="unidad" type="text" value="{{ old('unidad', $producto->unidad) }}" placeholder="kg, unidad, caja..."
                        class="w-full rounded border border-slate-300 focus:border-slate-500 focus:ring-slate-500">
                </div>
            </div>
            ...
            @include('partials.campos-precio-por-cantidad', ['producto' => $producto])

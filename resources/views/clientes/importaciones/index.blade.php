...
@section('contenido')
    <div class="mb-4">
        <a href="{{ route('clientes.index') }}" class="text-sm text-slate-500 hover:underline">&larr; Volver a Clientes</a>
    </div>
        ...
        @forelse ($importaciones as $importacion)
            <div class="p-4 flex items-center justify-between gap-4">
                ...
                <div class="flex items-center gap-3 shrink-0">
                    ...
                    @else
                        <form method="POST" action="{{ route('clientes.importaciones.deshacer', $importacion) }}"
                            onsubmit="return confirm('¿Deshacer esta importación? Los clientes creados se van a borrar y los actualizados van a volver a su estado anterior. Si editaste algo a mano después de importar, ese cambio también se va a perder. Esta acción no se puede deshacer.');">
                            @csrf

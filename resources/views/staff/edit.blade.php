        ...
        <form method="POST" action="{{ route('staff.update', $staff) }}" class="space-y-4">
            ...
            <div>
                ...
                <select id="role" name="role" required class="w-full rounded border border-slate-300 focus:border-slate-500 focus:ring-slate-500">
                    <option value="vendedor" @selected(old('role', $staff->role) === 'vendedor')>Vendedor</option>
                    <option value="cajero" @selected(old('role', $staff->role) === 'cajero')>Cajero</option>
                    <option value="cajero_vendedor" @selected(old('role', $staff->role) === 'cajero_vendedor')>Cajero vendedor (hace todo solo)</option>
                </select>
            </div>
            ...
            <label class="flex items-center gap-2 text-sm text-slate-600">
                <input type="checkbox" name="puede_cambiar_precio_venta" value="1" @checked(old('puede_cambiar_precio_venta', $staff->puede_cambiar_precio_venta)) class="rounded border border-slate-300">
